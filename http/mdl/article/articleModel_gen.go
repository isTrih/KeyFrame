...
func (m *defaultArticleModel) Update(ctx context.Context, data *Article) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Title, data.Content, data.RawContent, data.AuthorId, data.Status, data.Type, data.IpLocation, data.PublishTime, data.AiInsp, data.Insp, data.Id)
	}, keyframeArticleIdKey)
	return err
}
